    ...
    private fun buildSignals(classHierarchy: List<KSClassDeclaration>): SignalField? {
        ...
        signalHierarchy.forEach { iDecl ->
            iDecl.getDeclaredFunctions().forEach { signal ->
                val paramTypes = signal.parameters.map { p -> p.type.resolve() }
                val paramJvmTypes = signal.parameters.mapIndexed { index, p ->
                    JvmType.fromKSType(paramTypes[index], resolver.builtIns, logger, "signal-param: $p")
                }
                // The javaSignature here must match the signature generated by QtSignalProxy,
                // as this signature is used as the cachec lookup key
                val javaSignature = buildString {
